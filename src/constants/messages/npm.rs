...
pub const NPM_ERR_LOCKFILE_ONLY_FAILED_TEMPLATE: &str =
    "{} lockfile generation failed ({})";
pub const NPM_ERR_EXEC_FAILED_TEMPLATE: &str = "Failed to execute {}: {}";

pub const NPM_HINT_ERESOLVE: &str =
    "hint: peer dependency conflict — sentinel cannot generate a safe lockfile.\n\
     ...
     \x20      sentinel ci";
pub const YARN_HINT_ERESOLVE: &str =
    "hint: dependency conflict — sentinel cannot generate a safe lockfile.\n\
     \x20 1. Fix incompatible versions in package.json\n\
     \x20 2. Generate lockfile without installing:\n\
     \x20      yarn install --mode=update-lockfile\n\
     \x20 3. Verify and install through sentinel:\n\
     \x20      sentinel ci";
pub const PNPM_HINT_ERESOLVE: &str =
    "hint: dependency conflict — sentinel cannot generate a safe lockfile.\n\
     \x20 1. Fix incompatible versions in package.json\n\
     \x20 2. Generate lockfile without installing:\n\
...
pub const NPM_HINT_GENERATE_LOCKFILE_MANUALLY_TEMPLATE: &str =
    "hint: generate the lockfile manually with your package manager, then rerun sentinel:\n\
     \x20 {} install {}";
