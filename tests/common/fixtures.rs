...
pub(crate) fn integrity_fixture(input: &[u8]) -> IntegrityFixture {
    let bytes = Sha512::digest(input).to_vec();
    let integrity = format!("{SHA512_PREFIX}{}", B64.encode(&bytes));
    IntegrityFixture { bytes, integrity }
}

pub(crate) fn wrong_prefix_integrity(input: &[u8]) -> IntegrityFixture {
    let bytes = Sha512::digest(input).to_vec();
    let integrity = format!("{SHA256_PREFIX}{}", B64.encode(&bytes));
    IntegrityFixture { bytes, integrity }
}

pub(crate) fn hex_sha512(input: &[u8]) -> String {
    let bytes = Sha512::digest(input).to_vec();
    bytes.iter().fold(String::new(), |mut acc, value| {
        use std::fmt::Write;
        let _ = write!(acc, "{value:02x}");
        acc
    })
}
